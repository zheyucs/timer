const STORAGE_KEY = 'exam-timer-state-v1';
...
function renderCurrentTime() {
  currentTimeDisplay.textContent = formatDisplayTime(new Date()).toLowerCase();
}
...
function updateMetaTimes() {
  if (sessionStartWallClock) startTimeDisplay.textContent = formatDisplayTime(sessionStartWallClock);
  if (sessionEndWallClock) endTimeDisplay.textContent = formatDisplayTime(sessionEndWallClock);
}
...
function persistState() {
  const state = {
    ...
    minutes: Number(minutesInput.value || 25),
    currentDurationMs,
    elapsedMs: getElapsedMs(),
    isRunning,
    runStartedAt,
    sessionStartWallClock: sessionStartWallClock ? sessionStartWallClock.getTime() : null,
    sessionEndWallClock: sessionEndWallClock ? sessionEndWallClock.getTime() : null,
    savedAt: Date.now()
  };
...
function restoreState() {
  ...
  try {
    ...
    const minuteValue = Number(saved.minutes ?? 25);
    hoursInput.value = hourValue;
    minutesInput.value = minuteValue;

    currentDurationMs = Number(saved.currentDurationMs || (hourValue * 60 + minuteValue) * 60 * 1000 || 25 * 60 * 1000);
...
function setTimerFromSettings() {
  const totalMinutes = Math.max(0, Number(hoursInput.value || 0) * 60 + Number(minutesInput.value || 0));
  currentDurationMs = totalMinutes > 0 ? totalMinutes * 60 * 1000 : 25 * 60 * 1000;
...
function openModal() {
  examTitleInput.value = examTitleHeader.textContent === 'Exam Timer' ? '' : examTitleHeader.textContent;
